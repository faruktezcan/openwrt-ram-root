...
do_backup() { # create ram-root backup
  ...
  if ${BACKGROUND_BACKUP}; then
    ...
    eval "${cmd} && do_logger 'Info: backup created in ${SERVER}:${name}' || do_error 'Error: backup failed'" &
  else
    do_logger "Info: sending backup file"
    ${VERBOSE} && start_progress
    eval "${cmd}"; local result=$?
    ${VERBOSE} && kill_progress; printf $'\r'
    [[ ${result} -eq 0 ]] && do_logger "Info: backup created in ${SERVER}:${name}" || do_error "Error: backup failed"
  fi
...
do_restore() { # restore ram-root backup
  ...
  if ${backupExist}; then
    ...
    ${VERBOSE} && kill_progress; printf $'\r'
    [[ ${result} -eq 0 ]] \
      && do_logger "Info: backup file restored succesfully" \
      || do_error "Error: backup file restore failed"
  else
